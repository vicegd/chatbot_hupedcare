...
def _collector_lock_path(config):
    return os.path.join(config['storage']['data_folder'], ".collector.lock")


def _acquire_collector_lock(config):
    """
    Acquire an exclusive process lock so only one collector run can execute.
    """
    lock_path = _collector_lock_path(config)
    os.makedirs(os.path.dirname(lock_path), exist_ok=True)

    try:
        fd = os.open(lock_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
        with os.fdopen(fd, "w", encoding="utf-8") as lock_file:
            lock_file.write(json.dumps({"pid": os.getpid(), "started_at": int(time.time())}))
        return True
    except FileExistsError:
        logger.warning("Collector already running (lock file exists). Skipping this run.")
        return False


def _release_collector_lock(config):
    lock_path = _collector_lock_path(config)
    try:
        if os.path.exists(lock_path):
            os.remove(lock_path)
    except Exception as error:
        logger.warning(f"Could not remove collector lock file: {error}")


def _is_run_throttled(config, metadata):
    """
    Prevent aggressive scheduler loops from running the collector too frequently.
    """
    interval_from_config = config.get("collector", {}).get("min_interval_seconds", 3600)
    min_interval_seconds = int(os.getenv("COLLECTOR_MIN_INTERVAL_SECONDS", interval_from_config))
    now = int(time.time())
    last_run_ts = int(metadata.get("last_collector_run_ts", 0))

    if last_run_ts and (now - last_run_ts) < min_interval_seconds:
        wait_for = min_interval_seconds - (now - last_run_ts)
        logger.warning(
            f"Collector throttled. Last run was {now - last_run_ts}s ago. "
            f"Minimum interval is {min_interval_seconds}s. Try again in {wait_for}s."
        )
        return True

    metadata["last_collector_run_ts"] = now
...
def run_collector():
    ...
    if not _acquire_collector_lock(config):
        return

    if _is_run_throttled(config, metadata):
        _release_collector_lock(config)
        return
